...
function readJSON(folder, filename) {
    return __awaiter(this, void 0, void 0, function () {
        var fs, path, URL$1, __dirname, folderPath, filepath, result, indexFile, folderPath, filepath, _a, _b;
        return __generator(this, function (_c) {
            switch (_c.label) {
                case 0: return [4 /*yield*/, Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require('fs')); }).then(function (m) { return m.promises; })];
                case 1:
                    fs = _c.sent();
                    return [4 /*yield*/, Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require('path')); })];
                case 2:
                    path = _c.sent();
                    return [4 /*yield*/, Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require('url')); })];
                case 3:
                    URL$1 = (_c.sent()).URL;
                    __dirname = new URL$1(".", (typeof document === 'undefined' ? new (require('u' + 'rl').URL)('file:' + __filename).href : (document.currentScript && document.currentScript.src || new URL('index.cjs.js', document.baseURI).href))).pathname;
                    _c.label = 4;
                case 4:
                    _c.trys.push([4, 6, , 8]);
                    folderPath = path.resolve(__dirname, "..", folder);
                ...
                case 6:
                    _c.sent();
                    indexFile = resolveSync("@code-hike/lighter", { basedir: __dirname });
                    folderPath = path.resolve(indexFile, "..", "..", folder);
                    filepath = path.resolve(folderPath, filename);
                    _b = (_a = JSON).parse;
                    return [4 /*yield*/, fs.readFile(filepath, "utf8")];
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    return [4 /*yield*/, fetch("https://lighter.codehike.org/api/".concat(endpoint, "&v=").concat(LIGHTER_VERSION))];
                case 1:
                    r = _a.sent();
                    return [4 /*yield*/, r.json()];
                case 2: return [2 /*return*/, _a.sent()];
                case 3: return [4 /*yield*/, Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require('https')); })];
                case 4:
                    https = _a.sent();
                    options = {
                        host: "lighter.codehike.org",
                        path: "/api/".concat(endpoint, "&v=").concat(LIGHTER_VERSION),
                        method: "GET"
                    };
...
function getThemeColors(theme) {
    return __assign({ colorScheme: theme.type }, getColors(theme));
}
...
function getColors(theme) {
    var colors = {};
    for (var key in colorNamesToKeys) {
...
function getColor(theme, name) {
    ...
    var defaultColors = defaults[name];
...
function loadTheme(theme) {
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    if (!(typeof theme === "string")) return [3 /*break*/, 2];
                    return [4 /*yield*/, loadThemeByName(theme)];
                case 1:
                    _a = _b.sent();
                    return [3 /*break*/, 3];
                case 2:
                ...
                case 3:
                    rawTheme = _a;
                    return [2 /*return*/, toFinalTheme(rawTheme)];
            }
        });
    });
}
...
function toFinalTheme(theme) {
    ...
    var finalTheme = __assign(__assign({}, theme), { name: theme.name || "unknown-theme", type: getColorScheme(theme), settings: theme.settings || theme.tokenColors || [], colors: theme.colors || {} });
    var globalSetting = finalTheme.settings.find(function (s) { return !s.name && !s.scope; });
    if (globalSetting) {
        var _a = globalSetting.settings || {}, foreground = _a.foreground, background = _a.background;
...
function tokenize(code, grammar, colors) {
    ...
    return lines.map(function (line) {
        var _a = grammar.tokenizeLine2(line, stack), tokens = _a.tokens, ruleStack = _a.ruleStack;
        var newTokens = [];
        var tokenEnd = line.length;
        for (var i = tokens.length - 2; i >= 0; i = i - 2) {
            var tokenStart = tokens[i];
            var metadata = tokens[i + 1];
            newTokens.unshift({
...
function getStyle(metadata, colors) {
    var fg = (metadata & FOREGROUND_MASK) >>> FOREGROUND_OFFSET;
    // const bg = (metadata & BACKGROUND_MASK) >>> BACKGROUND_OFFSET;
    var style = {
        color: colors[fg]
    };
    var fs = (metadata & FONT_STYLE_MASK) >>> FONT_STYLE_OFFSET;
...
function aliasToLangData(alias) {
    var scope = aliasOrIdToScope[alias];
    if (!scope) {
        return undefined;
    }
    var id = scopeToLanguageData[scope].id;
    return {
        id: id,
        scopeName: scope
    };
...
function scopeToLangData(scope) {
    var data = scopeToLanguageData[scope];
    return data;
}

var sourceToGrammarPromise = new Map();
var shouldUseFileSystemPromise = undefined;
var shouldUseFileSystem = undefined;
function loadGrammarByScope(scope) {
    return __awaiter(this, void 0, void 0, function () {
        var lang, grammarPromise, promise_1, fetchPromise, subScopes, promise;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    if (sourceToGrammarPromise.has(scope)) {
                        return [2 /*return*/, sourceToGrammarPromise.get(scope)];
                    }
                    lang = scopeToLangData(scope);
                    if (!lang) {
                    ...
                    if (!(shouldUseFileSystem === undefined)) return [3 /*break*/, 2];
                    return [4 /*yield*/, shouldUseFileSystemPromise];
                case 1:
                    shouldUseFileSystem = _a.sent();
                    _a.label = 2;
                case 2:
                    if (shouldUseFileSystem) {
                        promise_1 = grammarPromise || loadGrammarFromFile(lang.path);
                        sourceToGrammarPromise.set(scope, promise_1);
                        return [2 /*return*/, promise_1];
                    }
                    fetchPromise = fetchJSON("grammars?lang=".concat(lang.id));
                    subScopes = lang.embeddedScopes;
                    subScopes.forEach(function (subScope) {
                        if (!sourceToGrammarPromise.has(subScope)) {
                            var subPromise = fetchPromise.then(function (gs) { return gs.find(function (g) { return g.scopeName === subScope; }); });
                            sourceToGrammarPromise.set(subScope, subPromise);
                        }
                    });
                    promise = fetchPromise.then(function (gs) { return gs.find(function (g) { return g.scopeName === scope; }); });
...
var registry = null;
function highlight(code, alias, themeOrThemeName) {
    if (themeOrThemeName === void 0) { themeOrThemeName = "dark-plus"; }
    return __awaiter(this, void 0, void 0, function () {
        var langData, onigLibPromise, grammarsPromise, theme, grammar, colorMap;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    langData = aliasToLangData(alias);
                    ...
                    if (!registry) {
                        onigLibPromise = main.exports.loadWASM(onig).then(function () { return ({
                        ...
                        registry = new main$1.exports.Registry({
                            ...
                            loadGrammar: function (scopeName) { return loadGrammarByScope(scopeName); }
                        });
                    }
                    grammarsPromise = registry.loadGrammar(langData.scopeName);
                    return [4 /*yield*/, loadTheme(themeOrThemeName)];
                case 1:
                    theme = _a.sent();
                    if (!theme) {
...
var UnknownLanguageError = /** @class */ (function (_super) {
    __extends(UnknownLanguageError, _super);
    function UnknownLanguageError(alias) {
        var _this = _super.call(this, "Unknown language: ".concat(alias)) || this;
        _this.alias = alias;
        return _this;
    }
    return UnknownLanguageError;
}(Error));
var UnknownThemeError = /** @class */ (function (_super) {
    __extends(UnknownThemeError, _super);
    function UnknownThemeError(theme) {
        var _this = _super.call(this, "Unknown theme: ".concat(theme)) || this;
        _this.theme = theme;
        return _this;
    }
